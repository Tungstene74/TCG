...
public class MouvCavalier implements Mouvement {
	...
	public Boolean estPossible(Piece piece, int new_x, int new_y, Plateau plateau) {
		...
		if (piece.mangeableOuNull(plateau,new_x,new_y)) {
			if ((new_x==x+2 & (new_y==y+1 | new_y==y-1)) | (new_x==x-2 & (new_y==y+1 | new_y==y-1)) | (new_y==y+2 & (new_x==x+1 | new_y==x-1))
					| (new_y==y-2 & (new_x==x+1 | new_y==x-1))) {
				b=true;
			}
		}
		return b;
